...
const OPERATOR_LONG_PRESS_MS = 3000;
const OPERATOR_AUTO_DISMISS_MS = 30000;
const ONBOARDING_FLAG = "garry-onboarding-seen";
const VISITOR_SIDE = "white"; // visitor plays white, Garry plays black
const STARTING_FEN_BOARD = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR";
// ── Module state ───────────────────────────────────────────────────────────
let chessService = null;
let currentState = { name: "connecting" };
let pollTimer = null;
...
function snapSide(snap) {
    if (snap.turn === "white" || snap.turn === "black")
        return snap.turn;
    const fenColor = (snap.fen.split(" ")[1] ?? "w").toLowerCase();
    return fenColor === "b" ? "black" : "white";
}
function isStartingPosition(fen) {
...
function findKingSquare(fen, side) {
...
function diffCameraVsFen(snap) {
    if (!snap.camera_board)
        return { count: 0 };
...
function isShowingIllegal() {
    if (firstDisagreementAt == null)
        return false;
    return Date.now() - firstDisagreementAt > ILLEGAL_DEBOUNCE_MS;
}
function diffFenForLastMove(prevFen, nextFen) {
    ...
    for (let rankIdx = 0; rankIdx < 8; rankIdx++) {
        for (let fileIdx = 0; fileIdx < 8; fileIdx++) {
            ...
            if (!n)
                ...
                filled.push(sq); // capture — piece replaced
        }
    }
...
function classifyOutcome(snap) {
    ...
    if (o === "white_won")
        return VISITOR_SIDE === "white" ? "win" : "loss";
    if (o === "black_won")
        return VISITOR_SIDE === "white" ? "loss" : "win";
    return "draw";
}
...
function snapshotToActiveState(snap) {
    if (snap.is_over)
        return { name: "game_over", snap, outcome: classifyOutcome(snap) };
    const turn = snapSide(snap);
    if (turn === VISITOR_SIDE)
        return { name: "your_turn", snap };
    return { name: "garry_turn", snap };
}
function deriveState(snap, previous) {
    ...
    if (previous.name === "idle" || previous.name === "onboarding")
        return previous;
...
function ev(tag, attrs = {}, ...children) {
...
function renderBoard(opts) {
    ...
    if (thinking)
        wrap.append(ev("div", { class: "thinking-outline board-pulse" }));
            ...
            if (piece) {
                ...
                if (url) {
                    ...
                    if (breathePiece === sqName) {
                        image.setAttribute("class", "piece-breathe");
                    }
                    cell.append(image);
                }
            }
...
// ── Frame renderers ────────────────────────────────────────────────────────
function statusRow(tone, label, pulsing = false) {
    const row = ev("div", {
        class: "status-row",
...
function pedestal(name, dim = false) {
...
function renderConnectionLost() {
    ...
    pauseLabel.append(ev("span", {
        class: "status-dot",
        style: `background:var(--status-loss)`,
    }), "Pause");
    center.append(pauseLabel);
    center.append(textNode("div", "connection-lost-headline", "Garry has lost connection to the board."));
    center.append(textNode("div", "connection-lost-sub", "The game will resume on its own as soon as we’re back. Your position is saved."));
    center.append(textNode("div", "connection-lost-attendant", "If this lasts more than a minute, please ask a gallery attendant."));
    state.append(center);
    return state;
}
function renderIdle() {
    ...
    center.append(cta);
    center.append(textNode("div", "cta-hint", "Touch to begin · a full game takes about 20 minutes"));
    state.append(operatorHotspot(), center);
    return state;
}
function renderOnboarding() {
    ...
    center.append(textNode("div", "onboarding-headline", "You play on the board.\nGarry plays on the board too."));
...
function renderPlay(snap, tone, baseLabel, pulsing, thinking, footerText, visitorActive) {
    const state = ev("div", { class: "state" });
    ...
    if (showIllegal && illegalAttempt) {
        if (illegalAttempt.from)
            illegalSquares.push(illegalAttempt.from);
        if (illegalAttempt.to)
            illegalSquares.push(illegalAttempt.to);
    }
    let label = baseLabel;
    let statusTone = tone;
...
function renderYourTurn(snap) {
    return renderPlay(snap, "your-turn", "Your move", false, false, "The board is watching · move a piece", true);
}
function renderGarryTurn(snap) {
    return renderPlay(snap, "thinking", "Garry is thinking", true, true, "The arm will move when ready", false);
}
function renderGameOver(snap, outcome) {
    const state = ev("div", { class: "state" });
    state.append(operatorHotspot());
    const presets = {
        win: {
    ...
    center.append(textNode("div", "gameover-eyebrow", o.label, `color:var(--status-${o.tone})`));
    ...
    robotCol.append(textNode("div", "gameover-cta-hint", "The arm returns the pieces home."));
    ...
    manualCol.append(textNode("div", "gameover-cta-hint", "Set the pieces yourself, then tap Play Garry."));
...
function closeOperatorSheet() {
    ...
    if (operatorEl) {
        const overlay = operatorEl._overlay;
        operatorEl.remove();
        overlay?.remove();
        operatorEl = null;
    }
...
// ── DOM utility ────────────────────────────────────────────────────────────
function textNode(tag, cls, text, extraStyle) {
    ...
    if (tag !== "div") {
        ...
        if (extraStyle)
            swap.setAttribute("style", extraStyle);
        while (el.firstChild)
            swap.append(el.firstChild);
        return swap;
    }
    return el;
}
...
// ── Game-flow actions ──────────────────────────────────────────────────────
const STARTING_SNAP = { fen: STARTING_FEN_BOARD + " w - - 0 1" };
...
let firstDisagreementAt = null;
let illegalAttempt = null;
...
let suppressPollUntilFreshAt = null;
let lastLoggedSnap = null;
let pollTickCount = 0;
function pollLog(snap) {
    pollTickCount++;
    const changed = !lastLoggedSnap ||
    ...
    console.log(`[halcyon] snap fen=${snap.fen}  turn=${snap.turn ?? "?"}  auto=${snap.auto}  cache_age=${ageS}  is_over=${snap.is_over ?? false}  event=${snap.event ?? "-"}${changed ? "  (changed)" : "  (heartbeat)"}`);
    lastLoggedSnap = {
        fen: snap.fen,
        turn: snap.turn,
        auto: snap.auto,
    ...
    if (snap.auto === false && (currentState.name === "your_turn" || currentState.name === "garry_turn")) {
        console.warn("[halcyon] auto mode is OFF on the server during a game — visitor moves will not be detected. Halcyon should set this via ensureAuto(); check that the previous call succeeded.");
    }
}
async function poll() {
    if (!chessService)
        return;
    try {
        const raw = await chessService.doCommand(Struct.fromJson({ "board-snapshot": true }));
        const snap = (raw ?? {});
        if (!snap.fen) {
        ...
        if (suppressPollUntilFreshAt != null) {
            ...
            if (isStale) {
                if (pollTickCount % 4 === 0) {
                    console.log(`[halcyon] poll: suppressing stale snap during new-game (is_over=${snap.is_over} cache_age=${cachedAt} need>${suppressPollUntilFreshAt})`);
                }
                pollTickCount++;
                return;
            }
